...
% Abstract
\section*{Abstract}
Vietnamese sentiment analysis is a challenging task due to the linguistic characteristics of the language and the need for robust automated classification in practical applications. This thesis investigates three deep learning architectures for Vietnamese binary sentiment classification: BiLSTM, fully fine-tuned PhoBERT, and a proposed Hybrid PhoBERT-BiLSTM model that combines a frozen, layer-truncated PhoBERT encoder with a trainable bidirectional LSTM layer. All models are trained under the same experimental pipeline and evaluated on the UIT-VSFC in-domain test set, the AIVIVN-2019 cross-domain e-commerce dataset, and a real-world YouTube comment dataset without retraining.

Experimental results show that PhoBERT achieves the highest overall performance and generalization, reaching 98.19\% Macro-F1 on the in-domain test set and 83.63--87.91\% on out-of-domain datasets. The proposed Hybrid PhoBERT-BiLSTM model retains most of this performance, achieving 97.58\% Macro-F1 in-domain while reducing inference latency by 20--29\% compared with PhoBERT. BiLSTM is the fastest model, but it generalizes less effectively across domains. These findings demonstrate a clear trade-off between predictive performance and inference efficiency in Vietnamese sentiment classification.

\noindent\textbf{Keywords:} Vietnamese sentiment analysis, sentiment classification, BiLSTM, PhoBERT, hybrid model
...
To enable a fair visual comparison of error patterns under a consistent sample size, Table~\ref{tab:confmat-uitvsfc} presents the confusion matrix for each model on a fixed 300-sample subset of the UIT-VSFC test set. The subset size was chosen to match the sample size used for the detailed classification reports on AIVIVN-2019 and the YouTube wild test set (approximately 320 comments). The subset was randomly drawn once and kept identical across models. All three models are evaluated under the same random seed (42).
...
\begin{table}[htbp]
    \centering
    \caption{Sentence-length category distribution across the three test sets (UIT-VSFC counts are after Neutral removal).}
...
\textbf{In-domain performance.} On the UIT-VSFC test set, PhoBERT achieves the highest Accuracy and Macro-F1 (98.20\% and 98.19\%, respectively), followed closely by Hybrid PhoBERT-BiLSTM (97.60\%, 97.58\%), with BiLSTM trailing both by 2.4--3.0 percentage points (Section~4.2). Confusion matrix analysis on the 300-sample subsets (seed 42) confirms that both pretrained-representation models make substantially fewer classification errors than BiLSTM.
...
Furthermore, the YouTube wild test set's labeling procedure (Section~3.2.3) discarded all comments on which the two LLM annotators disagreed, retaining only cases where both independently converged on the same label prior to human verification. Because inter-annotator disagreement is itself often correlated with genuine linguistic ambiguity --- sarcasm, mixed sentiment, or ambiguous phrasing being precisely the cases most likely to produce disagreement between two independent annotators --- the resulting 320-comment evaluation set should be understood as a \textbf{high-confidence subset} of YouTube comments rather than a representative sample of the full comment population on the source video. Consequently, the real-world performance figures reported in Section~4.6 likely represent an optimistic upper bound, and may not fully reflect model performance on the more ambiguous or contested portion of real-world social media sentiment that this filtering procedure excluded.
