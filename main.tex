...
\begin{table}[htbp]
    ...
    \begin{minipage}{\textwidth}
        \footnotesize
        \emph{Note:} The H0/A5 checkpoint size (414.8\,MB) differs slightly from the
        Hybrid PhoBERT-BiLSTM checkpoint size reported in Table~\ref{tab:modelsize}
        (416\,MB) for the same architecture. This discrepancy arises because the
        ablation checkpoints were saved from independent training runs using a
        different serialization routine than the main experiments, which additionally
        stored optimizer/scheduler state alongside the model weights; both figures
        reflect the same underlying architecture and parameter count.
    \end{minipage}
\end{table}
...
All experiments in this study were conducted on Google Colab's free tier, using a single NVIDIA Tesla T4 GPU (Section~3.6), which motivated the model-specific epoch budgets discussed in Section~\ref{subsec:epoch-justification}. While the convergence analysis in that section indicates these budgets did not constrain the reported results, the same hardware constraint precluded experimentation with larger PhoBERT variants (e.g., \texttt{phobert-large}) or more extensive hyperparameter search, either of which might further improve the reported results. Additionally, GPU memory consumption was not systematically measured, although total training time is reported in Table~\ref{tab:modelsize} (Section~4.5.2), limiting the completeness of the computational cost analysis relative to the inference-latency and parameter-count dimensions that were measured.
\section{Future Work}
