...
\textbf{Class Distribution.} Table~\ref{tab:youtube-dist} shows the resulting sentiment class distribution of the final 314-comment test set. Negative comments account for 63.69\%, while Positive comments account for 36.31\%. This imbalance is notably opposite to the class distribution previously expected from the tribute/documentary nature of the source video, which was assumed to elicit predominantly positive comments. This suggests that the filtering and labeling process --- particularly the exclusion of LLM-disagreed cases --- may have substantially altered the class distribution of the final test set relative to the raw comment population. The 314-comment subset should therefore be understood as a high-confidence, filtered subset rather than a representative sample of the source video's comment distribution.
\begin{table}[htbp]
    \centering
...
All experiments in this study were conducted on Google Colab's free tier, using a single NVIDIA Tesla T4 GPU (Section~3.6), which motivated the model-specific epoch budgets discussed in Section~\ref{subsec:epoch-justification}. While the convergence analysis in that section indicates these budgets did not constrain the reported results, the same hardware constraint precluded experimentation with larger PhoBERT variants (e.g., \texttt{phobert-large}) or more extensive hyperparameter search, either of which might further improve the reported results. Additionally, GPU memory consumption and total training time were not systematically measured (Section~4.5), limiting the completeness of the computational cost analysis relative to the inference-latency and parameter-count dimensions that were measured.
\section{Future Work}
